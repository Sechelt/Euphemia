...
DATAWOutputMessagesWidget::DATAWOutputMessagesWidget( QWidget *pParent )
...
DATAWOutputMessagesWidget::~DATAWOutputMessagesWidget()
{
    doSaveState();
}

void DATAWOutputMessagesWidget::slotMessage( ADDataEventGeneral *pEvent )
{
    if ( pEvent->pMessage->isNull() ) return;

    int nRow = rowCount();

    setRowCount( nRow + 1 );

    QTableWidgetItem *pItem = new QTableWidgetItem( pEvent->pMessage->getTypeText() );
    
    switch ( pEvent->pMessage->getType() )
    {
    ...
    setItem( nRow, 1, pItem = new QTableWidgetItem( pEvent->pMessage->getNumeric() ) );
    pItem->setFlags( Qt::ItemIsEnabled | Qt::ItemIsSelectable );

    setItem( nRow, 2, pItem = new QTableWidgetItem( pEvent->pMessage->getRequest() ) );
    pItem->setFlags( Qt::ItemIsEnabled | Qt::ItemIsSelectable );

    setItem( nRow, 3, pItem = new QTableWidgetItem( pEvent->pMessage->getText() ) );
    pItem->setFlags( Qt::ItemIsEnabled | Qt::ItemIsSelectable );
...
void DATAWOutputMessagesWidget::slotClear()
{
    setRowCount( 0 );
    vectorPaths.clear();
}

void DATAWOutputMessagesWidget::doSaveState()
{
...
void DATAWOutputMessagesWidget::doLoadState()
{
    QString s = metaObject()->className();
    if ( !objectName().isEmpty() ) s = s + "/" + objectName();
    s += "/column";

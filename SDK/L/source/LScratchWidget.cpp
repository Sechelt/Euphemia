...
EScratchSwatch::EScratchSwatch( QWidget *pParent )
    : QWidget( pParent )
{
}

void EScratchSwatch::setImage( const QImage &i )
{
    image = i;
    update();
}

void EScratchSwatch::paintEvent( QPaintEvent *pEvent )
{
    Q_UNUSED(pEvent);
...
LScratchWidget::LScratchWidget( QWidget *pParent )
    : QWidget( pParent )
{
    ...
    // main
    {
        ...
        pSwatch  = new EScratchSwatch( this );
...
void LScratchWidget::setMax( int n )
{
    if ( n < 1 ) return;
    if ( n > 100 ) n = 100;
    nMax = n;
...
void LScratchWidget::doAppend( const QImage &image )
{
...
void LScratchWidget::doEnableAdd( bool b )
{
    pAdd->setEnabled( b );
}

void LScratchWidget::slotSlider( int n )
{
    if ( n >= vectorImages.count() ) return;
    pSwatch->setImage( vectorImages.at( n ) );
}

void LScratchWidget::slotRemove()
{

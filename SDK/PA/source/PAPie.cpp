...
#define PAPieStartAngle 3
#define PAPieSpanAngle 4

PAPie::PAPie( PAPaint *pLayer )
    : PARectangle( pLayer )
{
}

void PAPie::doMove( PMouseEvent *pEvent ) 
{
...
void PAPie::doRelease( PMouseEvent *pEvent )
{
    Q_UNUSED( pEvent );
...
void PAPie::doPaint( QPainter *pPainter, bool bCommit )
{
...
void PAPie::doDrawState( const QPoint &point )
{
...
void PAPie::doSyncHandles()
{
    PARectangle::doSyncHandles();

    vectorHandles[PAPieStartAngle]->setCenter( pLayerView->mapFromScene( pointStart ) );
    vectorHandles[PAPieSpanAngle]->setCenter( pLayerView->mapFromScene( pointStop ) );
}

void PAPie::doMoveHandle( const QPoint &pointPos )
{
    Q_ASSERT( pHandle );
    ...
    // adjusting angles?
    if ( pHandle == vectorHandles[PAPieStartAngle] )
    {
    ...
    if ( pHandle == vectorHandles[PAPieSpanAngle] )
    {
        pHandle->setCenter( pointView );
        pointStop = pointPos;

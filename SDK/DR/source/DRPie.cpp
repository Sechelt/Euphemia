...
#define DRPieStartAngle 3
#define DRPieSpanAngle 4
/*
    if ( bBind )                                                                       
    {                                                                                  
...
DRPie::DRPie( DRDraw *pLayer, const QPointF &pointPos )
...
QPixmap DRPie::getIcon()
{
    return QPixmap( ":DR/Pie" );
}

LHandle *DRPie::getGrab( const QPointF &pointPos, bool bSizing )
{
    if ( bSizing ) 
    {
        if ( nSelectionState != SelectionStateStandard ) return nullptr;
        return vectorHandles.at( DRRectangleEnd );
    }

    return DRObject::getGrab( pointPos ); 
}

void DRPie::doMove( LHandle *pHandle, const QPointF &pointPos )
{
    Q_ASSERT( pHandle );
    ...
    // adjusting angles?
    if ( pHandle == vectorHandles[DRPieStartAngle] )
    {
    ...
    if ( pHandle == vectorHandles[DRPieSpanAngle] )
    {
        pHandle->setCenter( pointView );
        pointStop = pointPos;
...
QDomElement DRPie::doSave( QDomDocument *pdomDoc, QDomElement *pdomElemParent )
{
...
bool DRPie::doLoad( QDomElement *pdomElemThis )
{
    ...
    nEndAngle   = pdomElemThis->attribute( "AngleEnd", "180" ).toInt();

    return true;
}

void DRPie::doDraw( QPainter *pPainter )
{
    // apply context
    pPainter->setPen( pen );
    ...
    // draw shape
    pPainter->drawPie( r, nStartAngle * 16, nSpan * 16 );                         
...
void DRPie::doSyncHandles()
{
    DRRectangleBase::doSyncHandles();

    vectorHandles[DRPieStartAngle]->setCenter( getView()->mapFromScene( pointStart ) );
    vectorHandles[DRPieSpanAngle]->setCenter( getView()->mapFromScene( pointStop ) );
}

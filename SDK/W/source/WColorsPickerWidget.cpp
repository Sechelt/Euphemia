...
//
// WColorsSpectrumWidget
//
WColorsSpectrumWidget::WColorsSpectrumWidget( const QColor &color, QWidget *pParent )
    : QWidget( pParent )
{
    this->color = color;
...
void WColorsSpectrumWidget::setColor( const QColor &c )
{
    if ( color == c ) return;
    color =  c;
...
void WColorsSpectrumWidget::mousePressEvent( QMouseEvent *pEvent )
{
 ...
 */
void WColorsSpectrumWidget::paintEvent( QPaintEvent * )
{
 ...
 * \param y 
 * 
 * \return WColorsSpectrumWidget::Polar 
 */
WColorsSpectrumWidget::Polar WColorsSpectrumWidget::cartesianToPolar( qreal x, qreal y )
{
    // θ = tan-1 ( y / x )
...
//
// WColorsPickerSwatchWidget
//
WColorsPickerSwatchWidget::WColorsPickerSwatchWidget( const QColor &c, QWidget *pParent )
    : QWidget( pParent )
{
    color = c;
...
void WColorsPickerSwatchWidget::slotColor( const QColor &c )
{
    if ( color == c ) return;
    color =  c;
    update();
...
void WColorsPickerSwatchWidget::paintEvent( QPaintEvent * )
{
    QPainter painter( this );
...
// WColorsPickerWidget
//
WColorsPickerWidget::WColorsPickerWidget( const QColor &c, QWidget *pParent, bool bShowPaletteColor )
    : QWidget( pParent )
{
    int nH = 0;
    int nS = 0;
    ...
    // picker
    pSpectrum = new WColorsSpectrumWidget(color, this);
    ...
    // swatch
    pSwatch = new WColorsPickerSwatchWidget( color, this );
    pLayoutMain->addWidget( pSwatch );

    pLayoutTop->addLayout( pLayoutMain );

    // Palette Color (optional)
    // - WColorsWidget should never show in response to select a color for color palette - others - probably yes
    if ( bShowPaletteColor ) 
    {
        QVBoxLayout *pLayout = new QVBoxLayout();
        pLayout->addWidget( new QLabel( tr("User Defined"), this ) );

        pPalette = new WColorsWidget( this );
...
void WColorsPickerWidget::setColor( const QColor &c )
{
    slotPalette( c );
}
 ...
 * \param color 
 */
void WColorsPickerWidget::slotSpectrum( const QColor &c )
{
    int nHue    = 0;
    int nSat    = 0;
    int nVal    = 0;
...
void WColorsPickerWidget::slotPalette( const QColor &c )
{
    int nHue    = 0;
    int nSat    = 0;
    int nVal    = 0;
...
void WColorsPickerWidget::slotHue( int nH )
{
    int n  = 0;
    int nS = 0;
    int nV = 0;
...
void WColorsPickerWidget::slotSat( int nS )
{
    int nH = 0;
    int n  = 0;
    int nV = 0;
...
void WColorsPickerWidget::slotVal( int nV )
{
    int nH = 0;
    int nS = 0;
    int n  = 0;
...
void WColorsPickerWidget::slotAlp( int nA )
{
    int nH = 0;
    int nS = 0;
    int nV = 0;
